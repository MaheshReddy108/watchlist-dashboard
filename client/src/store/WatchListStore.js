...
class WatchlistStore {
  ...
  fetchData = () => {
    return axios
      .get(`http://${host}:3001/watchlist/${this.selectedUser}`, {
        headers: {
  ...
  handleDelete = symbol => {
    return axios
      .delete(`http://${host}:3001/watchlist/${this.selectedUser}/remove`, {
        headers: {
          "Content-Type": "application/json"
        },
        data: { symbol }
  ...
  handleUpdate = symbol => {
    return axios
      .get(`http://${host}:3001/symbol/${symbol}`)
      .then(response => {
